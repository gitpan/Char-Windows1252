...
######################################################################
#
# Windows1252 - Source code filter to Windows-1252 script
#
...
my $function_ord;         # ord()   to ord()   or Windows1252::ord()
my $function_ord_;        # ord     to ord     or Windows1252::ord_
my $function_reverse;     # reverse to reverse or Windows1252::reverse
...
sub Windows1252::escape_script {
    ...
    if (m/^ use Ewindows1252(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Ewindows1252 %s;\n%s", $Ewindows1252::VERSION, $use_re_eval); # require run-time routines version

        # use Windows1252 version qw(ord reverse);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        $function_reverse = 'reverse';
        if (s/^ \s* use \s+ Windows1252 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list !~ m/\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Windows1252::ord'     if m/\A ord \z/oxms;
                    $function_ord_    = 'Windows1252::ord_'    if m/\A ord \z/oxms;
                    $function_reverse = 'Windows1252::reverse' if m/\A reverse \z/oxms;
                }
            }
        }
    }
...
# functions of package Ewindows1252
    elsif (m{\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  }oxgc) { $slash = 'm//'; return $1;                    }
    elsif (m{\G \b bytes::substr \b (?! \s* => )                }oxgc) { $slash = 'm//'; return 'substr';              }
    elsif (m{\G \b chop \b          (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Ewindows1252::chop';         }
    elsif (m{\G \b bytes::index \b  (?! \s* => )                }oxgc) { $slash = 'm//'; return 'index';               }
    elsif (m{\G \b Windows1252::index \b   (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Windows1252::index';         }
    elsif (m{\G \b index \b         (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Ewindows1252::index';        }
    elsif (m{\G \b bytes::rindex \b (?! \s* => )                }oxgc) { $slash = 'm//'; return 'rindex';              }
    elsif (m{\G \b Windows1252::rindex \b  (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Windows1252::rindex';        }
    elsif (m{\G \b rindex \b        (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Ewindows1252::rindex';       }
    elsif (m{\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ewindows1252::lc';           }
    elsif (m{\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ewindows1252::lcfirst';      }
    elsif (m{\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ewindows1252::uc';           }
    elsif (m{\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ewindows1252::ucfirst';      }
    elsif (m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ewindows1252::fc';           }
    ...
    elsif (m{\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ewindows1252::chr';               }
    ...
    elsif (m{\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Ewindows1252::glob';              }
    elsif (m{\G \b lc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ewindows1252::lc_';               }
    elsif (m{\G \b lcfirst \b    (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ewindows1252::lcfirst_';          }
    elsif (m{\G \b uc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ewindows1252::uc_';               }
    elsif (m{\G \b ucfirst \b    (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ewindows1252::ucfirst_';          }
    elsif (m{\G \b fc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Ewindows1252::fc_';               }
    ...
    elsif (m{\G \b chr \b           (?! \s* => )                      }oxgc) { $slash = 'm//'; return 'Ewindows1252::chr_';              }
    elsif (m{\G \b bytes::ord \b    (?! \s* => )                      }oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (m{\G \b ord \b           (?! \s* => )                      }oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (m{\G \b glob \b          (?! \s* => )                      }oxgc) { $slash = 'm//'; return 'Ewindows1252::glob_';             }
...
# functions of package Ewindows1252
        ...
        elsif ($string =~ m{\G \b chop \b                                      }oxgc) { $e_string .= 'Ewindows1252::chop';    $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::index \b                              }oxgc) { $e_string .= 'index';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b Windows1252::index \b                               }oxgc) { $e_string .= 'Windows1252::index';    $slash = 'm//'; }
        elsif ($string =~ m{\G \b index \b                                     }oxgc) { $e_string .= 'Ewindows1252::index';   $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::rindex \b                             }oxgc) { $e_string .= 'rindex';         $slash = 'm//'; }
        elsif ($string =~ m{\G \b Windows1252::rindex \b                              }oxgc) { $e_string .= 'Windows1252::rindex';   $slash = 'm//'; }
        elsif ($string =~ m{\G \b rindex \b                                    }oxgc) { $e_string .= 'Ewindows1252::rindex';  $slash = 'm//'; }
        elsif ($string =~ m{\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ewindows1252::lc';      $slash = 'm//'; }
        elsif ($string =~ m{\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ewindows1252::lcfirst'; $slash = 'm//'; }
        elsif ($string =~ m{\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ewindows1252::uc';      $slash = 'm//'; }
        elsif ($string =~ m{\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ewindows1252::ucfirst'; $slash = 'm//'; }
        elsif ($string =~ m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ewindows1252::fc';      $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ewindows1252::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Ewindows1252::glob';              $slash = 'm//'; }
        elsif ($string =~ m{\G \b lc \b                                              }oxgc) { $e_string .= 'Ewindows1252::lc_';               $slash = 'm//'; }
        elsif ($string =~ m{\G \b lcfirst \b                                         }oxgc) { $e_string .= 'Ewindows1252::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b uc \b                                              }oxgc) { $e_string .= 'Ewindows1252::uc_';               $slash = 'm//'; }
        elsif ($string =~ m{\G \b ucfirst \b                                         }oxgc) { $e_string .= 'Ewindows1252::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b fc \b                                              }oxgc) { $e_string .= 'Ewindows1252::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b chr \b                                             }oxgc) { $e_string .= 'Ewindows1252::chr_';              $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::ord \b                                      }oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ m{\G \b ord \b                                             }oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ m{\G \b glob \b                                            }oxgc) { $e_string .= 'Ewindows1252::glob_';             $slash = 'm//'; }
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Ewindows1252::uc($char[$i]) ne Ewindows1252::fc($char[$i]))) {
...
Windows1252 - Source code filter to escape Windows-1252

=head1 SYNOPSIS

  use Windows1252;
  use Windows1252 version;         --- require version
...
You need write 'use Windows1252;' in your script.
  ...
  Before      You do
  ---------------------------------
  (nothing)   use Windows1252;
  ---------------------------------

=head1 Calling 'Ewindows1252::ignorecase()' (Windows1252.pm provides)

Windows1252.pm applies calling 'Ewindows1252::ignorecase()' instead of /i modifier.
...
=head1 Un-Escaping \ Of \N, \p, \P and \X (Windows1252.pm provides)

Windows1252.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Windows1252.pm and Ewindows1252.pm provide)
...
=head1 Un-Escaping bytes::* Functions (Windows1252.pm provides)

Windows1252.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Ewindows1252.pm provides)

Ewindows1252.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Windows1252' }" at head.
Store the standard module modified for Windows1252 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Windows1252 and change
'use utf8;' to 'use Windows1252;' in its. You need help yourself for now.
...
Contrasting is Encode module and describing "use Windows1252;" on this software,
