...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Ewindows1252::uc($char[$i]) ne Ewindows1252::lc($char[$i]))) {
            $char[$i] = '[' . Ewindows1252::uc($char[$i]) . Ewindows1252::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Windows1252/Windows1252/JA.pm
...
Definitions in Ewindows1252.pm.
  ...
  @{Ewindows1252::eV}             qr{(?:[^\x0C\x0A\x0D])}
