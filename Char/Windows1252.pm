...
######################################################################
#
# Char::Windows1252 - Source code filter to escape Windows-1252 script
#
# http://search.cpan.org/dist/Char-Char::Windows1252/
#
...
BEGIN { CORE::require Char::Ewindows1252; }
...
sub unimport {}
sub Char::Windows1252::escape_script;
...
my $function_ord;         # ord()   to ord()   or Char::Windows1252::ord()
my $function_ord_;        # ord     to ord     or Char::Windows1252::ord_
my $function_reverse;     # reverse to reverse or Char::Windows1252::reverse
my $function_getc;        # getc    to getc    or Char::Windows1252::getc
...
END
    ...
    print Char::Windows1252::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Ewindows1252::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Windows1252::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Ewindows1252::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        exit Char::Ewindows1252::_systemx(
...
sub Char::Windows1252::escape_script {
    ...
    my $fh = gensym();
    Char::Ewindows1252::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Ewindows1252(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Ewindows1252 %s;\n", $Char::Windows1252::VERSION); # require run-time routines version

        # use Char::Windows1252 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Windows1252 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Windows1252::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Windows1252.pm}} @INC;
...
****************************************************
              ...
              CONFLICT Char/Windows1252.pm FILE
...
END
                    ...
                    die "Script $0 expects Char/Windows1252.pm $version, but @{[__FILE__]} is version $Char::Windows1252::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Ewindows1252.pm $version, but \\\$Char::Ewindows1252::VERSION is \$Char::Ewindows1252::VERSION" if \$Char::Ewindows1252::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Windows1252::VERSION) {
                    die "Script $0 required Char/Windows1252.pm $version, but @{[__FILE__]} is only version $Char::Windows1252::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Windows1252::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Windows1252::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Windows1252::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Windows1252::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# escape Windows-1252 part of script
sub escape {
...
# functions of package Char::Ewindows1252
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Windows1252::re_t=%s; while($Char::Windows1252::re_t =~ %s){%s local $^W=0; local $Char::Windows1252::re_r=%s; %s$Char::Windows1252::re_t="$`$Char::Windows1252::re_r$'"; pos($Char::Windows1252::re_t)=length "$`$Char::Windows1252::re_r"; } return $Char::Windows1252::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Windows1252::re_r=eval $Char::Windows1252::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Windows1252::re_r=%s; %s"%s$Char::Windows1252::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Windows1252::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Windows1252::re_r=%s; %s%s="$`$Char::Windows1252::re_r$'"; pos(%s)=length "$`$Char::Windows1252::re_r"; $Char::Windows1252::re_n++} return %s$Char::Windows1252::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Windows1252::re_r=%s; %s%s="%s$Char::Windows1252::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Windows1252::re_r=%s; %s%s="%s$Char::Windows1252::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Windows1252::re_r=eval $Char::Windows1252::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Windows1252.pm and Char/Ewindows1252.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Windows1252;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Windows1252 ver.sion;             --- require minimum version
  use Char::Windows1252 ver.sion.0;           --- expects version (match or die)

  # "no Char::Windows1252;" not supported

  or

  $ perl Char/Windows1252.pm Windows-1252_script.pl > Escaped_script.pl.e
...
Char::Windows1252 software is "middleware" between perl interpreter and your Perl script
...
By "use Char::Windows1252;", it automatically interpret your script as Windows-1252. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/Windows1252.pm, Char/Ewindows1252.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Ewindows1252.pm
works well.

   in Char/Ewindows1252.pm
   #
...
=head1 Software Composition
   ...
   Char/Windows1252.pm               --- source code filter to escape Windows-1252
   Char/Ewindows1252.pm              --- run-time routines for Char/Windows1252.pm
...
You need write 'use Char::Windows1252;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::Windows1252;
  ---------------------------------

=head1 Calling 'Char::Ewindows1252::ignorecase()' (Char/Windows1252.pm provides)

Char/Windows1252.pm applies calling 'Char::Ewindows1252::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Ewindows1252::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Ewindows1252.pm provides)
...
The character classes are redefined as follows to backward compatibility.
  ...
  Before        After
  ---------------------------------------------------------------
   .            ${Char::Ewindows1252::dot}
                ${Char::Ewindows1252::dot_s}    (/s modifier)
  \d            [0-9]
  \s            \s
  \w            [0-9A-Z_a-z]
...
Also POSIX-style character classes.
  ...
  [:^lower:]    ${Char::Ewindows1252::not_lower}
                ${Char::Ewindows1252::not_lower_i}    (/i modifier)
  ...
  [:^upper:]    ${Char::Ewindows1252::not_upper}
                ${Char::Ewindows1252::not_upper_i}    (/i modifier)
...
Definitions in Char/Ewindows1252.pm.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Windows1252.pm provides)

Char/Windows1252.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Windows1252.pm and Char/Ewindows1252.pm provide)

Insert 'Char::Ewindows1252::' at head of function name. Char/Ewindows1252.pm provides your script Char::Ewindows1252::*
functions.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Ewindows1252::lc        Character
  lcfirst     Char::Ewindows1252::lcfirst   Character
  uc          Char::Ewindows1252::uc        Character
  ucfirst     Char::Ewindows1252::ucfirst   Character
  fc          Char::Ewindows1252::fc        Character
  chr         Char::Ewindows1252::chr       Character
  glob        Char::Ewindows1252::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Functions (Char/Windows1252.pm provides)

Char/Windows1252.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Ewindows1252.pm provides)

Char/Ewindows1252.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::Windows1252' }" at head.
Store the standard module modified for Char::Windows1252 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::Windows1252 and change
'use utf8;' to 'use Char::Windows1252;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Ewindows1252.pm provides these
  functions.
...
Char::Windows1252 software remains one language and one interpreter.
