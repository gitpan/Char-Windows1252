...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Ewindows1252::uc($char[$i]) ne Char::Ewindows1252::lc($char[$i]))) {
            $char[$i] = '[' . Char::Ewindows1252::uc($char[$i]) . Char::Ewindows1252::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Windows1252/Char::Windows1252/JA.pm
...
Definitions in Char/Ewindows1252.pm.
  ...
  @{Char::Ewindows1252::eV}             qr{(?:[^\x0C\x0A\x0D])}
