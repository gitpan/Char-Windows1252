...
BEGIN {
    ...
    sub Ewindows1252::binmode(*;$);
    sub Ewindows1252::open(*;$@);
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Ewindows1252::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Ewindows1252::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Ewindows1252::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Ewindows1252::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Ewindows1252::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Ewindows1252::uc($_[0]) =~ m{\A $pattern \z}xms };
...
This module is a run-time routines of the Windows1252.pm.
Because the Windows1252.pm automatically uses this module, you need not use directly.
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Ewindows1252::split returns
...
=item Chop string
  ...
  The Ewindows1252::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Ewindows1252::index($string,$substr,$position);
  $pos = Ewindows1252::index($string,$substr);
  ...
  the Ewindows1252::index function returns -1.

=item Reverse index string

  $pos = Ewindows1252::rindex($string,$substr,$position);
  $pos = Ewindows1252::rindex($string,$substr);

  Works just like Ewindows1252::index except that it returns the position of the last
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Ewindows1252::binmode(FILEHANDLE, $disciplines);
  Ewindows1252::binmode(FILEHANDLE);
  Ewindows1252::binmode($filehandle, $disciplines);
  Ewindows1252::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Ewindows1252::open(FILEHANDLE, $mode, $expr);
  $rc = Ewindows1252::open(FILEHANDLE, $expr);
  $rc = Ewindows1252::open(FILEHANDLE);
  $rc = Ewindows1252::open(my $filehandle, $mode, $expr);
  $rc = Ewindows1252::open(my $filehandle, $expr);
  $rc = Ewindows1252::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Ewindows1252::open($fh, ">logfile")     # $fh is autovivified
  ...
  Ewindows1252::open my $fh, ">logfile" or die ...
  ...
  Ewindows1252::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Ewindows1252::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Ewindows1252::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Ewindows1252::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Ewindows1252::open(FH, "<:crlf:encoding(Windows1252)", $path) or die "can't open $path: $!";
